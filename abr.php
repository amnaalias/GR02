...
// Get students with audio from lecturer database
$members = getStudentsWithAudio($group);
...
<head>
    ...
    <title>ABR Detection | Group <?php echo htmlspecialchars($group); ?></title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .audio-card .audio-header .audio-icon {
            font-size: 2.5rem;
            color: #ff9f4a;
        }
        
        .audio-card .audio-header .name {
            font-weight: bold;
            font-size: 1.1rem;
            color: #00d2ff;
        }
        
        .audio-card .audio-header .matric {
            color: #888;
            font-size: 0.85rem;
        ...
        .audio-card .audio-details .detail-item .label {
            color: #888;
            font-size: 0.75rem;
            text-transform: uppercase;
        }
        
        .audio-card .audio-details .detail-item .value {
            font-size: 1rem;
            font-weight: bold;
            margin-top: 3px;
        }
        ...
        @media (max-width: 500px) {
            ...
            .stat-item { width: 100%; }
            .audio-card .audio-details { grid-template-columns: 1fr; }
        }
    </style>
</head>
<body>
...
<nav class="sidebar" id="sidebar">
    ...
    <ul class="sidebar-menu">
        ...
        <li><a href="abr.php?group=<?php echo urlencode($group); ?>" class="active"><i class="fas fa-music"></i> ABR Detection</a></li>
...
<main class="main-content">
    ...
    <?php if (empty($members)): ?>
        <div class="empty-state">
            ...
            <p>No audio files have been uploaded or analyzed for this group.</p>
        </div>
    <?php else: ?>
...
<script>
    ...
    function analyzeAudio(matricNo, audioPath) {
        ...
        fetch('analyze_audio.php', {
            ...
            body: 'matric_no=' + encodeURIComponent(matricNo) + '&audio_path=' + encodeURIComponent(audioPath)
        })
